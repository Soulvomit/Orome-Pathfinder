...
FindPath:
	...
	push qword 0			;push Grid.PStartNode@72 as qword
	mov rax, [rbp + 40]		;intercept arg Grid.PMap@64
	push rax				;push arg as qword				
	mov rax, [rbp + 64]		;intercept arg Grid.POutBuffer@56
	push rax				;push arg as qword
	mov rax, [rbp + 72]		;intercept arg Grid.OutBufferSize@54
	push ax					;push arg as word
	mov rax, [rbp + 56]		;intercept arg Grid.MapHeight@52
	push ax					;push arg as word
	mov rax, [rbp + 48]		;intercept arg Grid.MapWidth@50
	push ax					;push arg as word
	push r9w				;intercept arg Grid.TargetY@48
	push r8w				;intercept arg Grid.TargetX@46
	push dx					;intercept arg Grid.StartY@44
	push cx					;intercept arg Grid.StartX@42
	mov rax, [rbp + 80]		;intercept arg Grid.IncludeDiagonals@40
	...
	mov ax, 4				;mov ax, 4 
	.include_diagonals:
	push word ax			;Grid.IncludeDiagonals@40
	push qword 1			;NodeOpenList.Count@32
	...
	.alloc:									;entry point of stack alloction loop
		...
		jz .find_path						;if last node, terminate alloc loop
			...
			;construct node
			push r8w						;Node.Position.Y: 	offset = 8*6+6	= 54
			push r9w						;Node.Position.X: 	offset = 8*6+4 	= 52
			mov cl, byte [r11+r10]			;store resistance as byte
			push cx 						;Node.Resistance: 	offset = 8*6+2 	= 50
